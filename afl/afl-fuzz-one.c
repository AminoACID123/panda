...
iut_init_stage:
  ...
  while (1) {
    ...
    for (i = q->message_cnt - n; i < q->message_cnt; ++i) {
      handle_message(afl, q, q->messages[i]->data, q->messages[i]->size);
    }
  ...
  if (unlikely(has_exec_fail_sig(afl->fsrv.trace_bits))) {
    FATAL("IUT crashes during init: %s", buzzer->crash_message);
  }
