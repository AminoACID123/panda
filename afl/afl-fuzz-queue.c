...
void add_to_queue(afl_state_t *afl, char *fname, u32 len, u8 passed_det) {
  ...
  q->trace_mini = NULL;
  // q->bt_state.le_conn = &q->bt_state.conns[LE_HANDLE];
  // q->bt_state.bredr_conn = &q->bt_state.conns[BREDR_HANDLE];
...
void queue_testcase_get(afl_state_t *afl, struct queue_entry *q) {
  ...
  if (unlikely(!q->loaded)) {
    ...
    while (unlikely(
      ...
      queue_entry_free_messages(old_cached);
      old_cached->loaded = 0;
