...
void show_stats_normal(afl_state_t *afl) {
  ...
  char val_buf[8][STRINGIFY_VAL_SIZE_MAX];
...
#define SP20 SP10 SP10
  ...
  if (!afl->non_instrumented_mode &&
      (afl->last_find_time || afl->resuming_fuzz || afl->queue_cycle == 1 || afl->crash_mode)) {
  ...
  if (afl->cpu_core_count) {
    ...
    if (!afl->no_cpu_meter_red && cur_utilization >= 150) { cpu_color = (char*)cLRD; }
...
void show_init_stats(afl_state_t *afl) {
  ...
  char val_bufs[4][STRINGIFY_VAL_SIZE_MAX];
