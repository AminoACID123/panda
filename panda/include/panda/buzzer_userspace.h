...
#define BZ_HYPERCALL_SEND               20          /* The Bluetooth Host wants to send something */
#define BZ_HYPERCALL_RECV               21          /* The Bluetooth Host wants to recv something */
#define BZ_HYPERCALL_WAIT               22          /* The Bluetooth Host is waiting for packet   */
#define BZ_HYPERCALL_PRINT              23          /* Print somethin                             */
#define BZ_HYPERCALL_PANIC              24          /* Panic (some annomoly has been detected)    */
#define BZ_HYPERCALL_GUEST_ERROR        25          /* Guest encounters some error during setup   */
#define BZ_HYPERCALL_REQ_HARNESS_INFO   26          /* Guest is requesting harness info for fuzz  */
#define BZ_HYPERCALL_REQ_FILE           27          /* Guest is requesting necessary file for fuzz*/
...
static inline void bz_hypercall(uintptr_t rax, uintptr_t rbx, 
    uintptr_t rcx, uintptr_t rdx, uintptr_t rdi, uintptr_t rsi) {
  ...
  asm __volatile__
      ("push %%rax \t\n\
       ...
       "
      : /* no output registers */
      : "r" (rax), "r" (rbx), "r" (rcx), "r" (rdx), "r" (rdi), "r" (rsi) /* input operands */
...
static inline void bz_hypercall(uintptr_t eax, uintptr_t ebx, 
    uintptr_t ecx, uintptr_t edx, uintptr_t edi, uintptr_t esi) {
  ...
  asm __volatile__
      ("push %%eax \t\n\
       ...
       "
      : /* no output registers */
      : "r" (eax), "r" (ebx), "r" (ecx), "r" (edx), "r" (edi), "r" (esi) /* input operands */
