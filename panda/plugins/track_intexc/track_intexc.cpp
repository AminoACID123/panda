...
string state_name[] = {"unknown",         "user",      "kernel",
                       "maybe_interrupt", "interrupt", "maybe_exception",
                       "exception"};
...
extern "C" bool check_in_interrupt() { return (current_state == Interrupt); }

extern "C" bool check_in_exception() { return (current_state == Exception); }
...
void check_transition(IEState old_state, IEState new_state) {
  ...
  if (debug) {
    cout << "instr = " << dec << rr_get_guest_instr_count() << hex
         << " Transition from " << state_name[old_state] << " -> "
...
void before_block_exec(CPUState *cpu, TranslationBlock *tb) {
  ...
  if (current_state == MaybeInterrupt || current_state == MaybeException) {
    ...
    if (pc != pc_check) {
      ...
      if (debug)
        printf("entering %s . instr=%" PRId64 " old_pc=%" PRIx64
               " new_pc=%" PRIx64 " number=%x\n",
               ((is_int) ? "interrupt" : "exception"),
               rr_get_guest_instr_count(), (uint64_t)pc_check, (uint64_t)pc,
  ...
  if (debug) {
    cout << "pc = " << hex << pc << " ASID = " << hex << asid
         << " ESP = " << ESP;
    cout << " in_kernel = " << in_kernel;
    if (old_state == current_state)
      cout << " state = " << state_name[current_state];
    else
      cout << " old_state,current_state = " << state_name[old_state] << ","
...
int insn_exec_callback(CPUState *cpu, target_ulong pc) {
  ...
  if (debug)
    cout << "iret cr3 = " << hex << panda_current_asid(cpu) << " ESP = " << ESP
         << " retto = " << retto << "\n";
  ...
  if (iret_stack.size() == 0) {
    if (debug)
      cout << "iret_stack is empty\n";
  } else {
    // check the kernel and current asid stack
    if (iret_stack[0].size() == 0) {
      if (debug)
        cout << "iret_stack for kernel stack is empty\n";
    } else {
    ...
    if (iret_stack[asid].size() == 0) {
      if (debug)
        cout << "iret_stack for asid=" << hex << asid << " is empty\n";
    } else {
      auto ie = iret_stack[asid].back();
      if (retto == ie.old_pc) {
        if (debug)
          cout << "Top of asid=" << hex << asid
               << " stack matches retto is_int = " << ie.is_int << "\n";
        // restore state to site of int/exc
        current_state = ie.state;
  ...
  if (iret_resolved)
    ...
    cout << "iret NOT resolved\n";

  num_irets++;
  if (debug)
    cout << dec << num_irets_resolved << " irets resolved out of " << num_irets
         << "\n";
...
#else
  std::cerr << PANDA_MSG "ERROR track_intexc plugin not implemented for "
                         "anything other than i386 and x86_64"
            << std::endl;
  return false;
#endif
  return true;
