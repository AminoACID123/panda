...
typedef struct afl_state {
  ...
  message_t*  message_list_scratch[MESSAGE_LIST_SCRATCH_LEN];
...
#define N_FUZZ_SIZE (1 << 21)
  ...
  char stage_name_buf[STAGE_BUF_SIZE];    /* reused stagename buf with len 64 */
...
void add_to_queue(afl_state_t *, char *, u32, u8);
...
u8 save_if_interesting(afl_state_t*, struct queue_entry*, u8);
...
u8   calibrate_case(afl_state_t *, struct queue_entry *, u32);
u8   common_fuzz_stuff(afl_state_t *, struct queue_entry*, u8);
...
static inline u32 prob_select(afl_state_t* afl, double* probs, u32 len) {

  u32 i = 0;
  double prob = rand_next_percent(afl);
  while (prob > 0 && i < len) {
    prob -= probs[i++];
  }
  return i - 1;
...
static inline void rand_bytes(afl_state_t* afl, void* out, u32 len) {
  ck_read(afl->fsrv.dev_urandom_fd, out, len, "/dev/urandom");
}
...
void bt_state_copy(bt_state_t *, bt_state_t *);
void bt_state_update_from_evt(bt_state_t *, bt_hci_evt_hdr *);
void bt_state_update_from_cmd(bt_state_t *, bt_hci_cmd_hdr *);
...
void queue_testcase_get(afl_state_t *afl, struct queue_entry *q);
