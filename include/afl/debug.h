...
#define ck_fwrite(f, buf, len, fn)                                             \
  do {                                                                         \
    ...
    do {                                                                       \
                                                                               \
      s32 _res = fwrite((buf) + _off, 1, _len, _f);                            \
...
#define ck_fread(f, buf, len, fn)                                              \
  do {                                                                         \
                                                                               \
    FILE *_f = (FILE *)(f);                                                    \
    s32 _len = (s32)(len);                                                     \
    s32 _res = fread(buf, 1, _len, _f);                                        \
    if (_res != _len)                                                          \
      PFATAL("Short read from %s", fn);                                        \
                                                                               \
  } while (0)

#define ck_write3(fd, buf, len)                                                \
  do {                                                                         \
    u32 _len = (len);                                                          \
    s32 _res = write(fd, buf, _len);                                           \
    if (_res != _len)                                                          \
      PFATAL("Short write: %d", _res);                                                   \
  } while (0)

#define ck_read3(fd, buf, len)                                                 \
  do {                                                                         \
    u32 _len = (len);                                                          \
    s32 _res = read(fd, buf, _len);                                            \
    if (_res != _len)                                                          \
      PFATAL("Short read: %d", _res);                                                    \
  } while (0)
